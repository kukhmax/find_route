...
def dfs_paths(graph, start, goal):
    ...
    из одного города в другой. Вфриант посещения
    одного и того же города олее одного раза,
    не рассматривается.
    """
    stack = [(start, [start])]
    while stack:
...
def get_routes(request, form):
    ...
    for route in right_ways:
        ...
        tmp['trains'] =[]
        total_time = 0
        for i in range(len(route) - 1):   # проходим по всему набору городов в каком-то маршруте
            qs = all_trains[(route[i], route[i + 1])]
            q = qs[0]  #  получаем первый экземпляр поезда
            total_time += q.travel_time  # прибаляем время к общему
            tmp['trains'].append(q)  # добавляем поезд в список 
        tmp['total_time'] = total_time  # добавляем общее время к этому поезду
    ...
    if len(routes) == 1:
        ...
        times =[set(r['total_time'] for r in routes)]
        times = sorted(times)
        for time in times:
            for route in routes:
